...
const UseFecthData = () => {
  const { data, isLoading, error } = useGetDataListQuery(
    createSearchQuery(useAppSelector((state) => state.data))
  );
  ...
  function getPhone(result: any): TableRowProps["phone"] {
    ...
    else return result.to_number.replace(
        /^(7)(\d{3})(\d{3})(\d{2})(\d{2})$/,
        "+$1 ($2) $3-$4-$5"
      );;
  }
  ...
  if (data && !isLoading) {
    const allResults: TableRowProps[] = data.results.map(
      (result: any): TableRowProps => {
        return {
          id: result.id,
